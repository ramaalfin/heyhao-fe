...
export default function DiscoverPage() {
  ...
  const {
    data: groupData,
    isLoading: isPendingGroup,
    error: errorGroup,
  } = useDiscoverGroup(tab === "GROUP", query ?? undefined);
  const {
    data: peopleData,
    isLoading: isPendingPeople,
    error: errorPeople,
  } = useDiscoverPeople(tab === "PEOPLE", query ?? undefined);
          ...
          <section id="Featured-Groups" className="flex flex-col gap-[12px]">
            ...
            {!isPendingGroup &&
              ...
              (groupData as any).length === 0 && (
                <p className="text-center text-heyhao-secondary">
                  No groups found
                </p>
              )}
        ...
        {tab === "PEOPLE" && (
          <section id="ContainerPeople" className="w-full p-[30px]">
            <div id="Result-People" className="flex flex-col gap-4">
              {!isPendingPeople &&
                peopleData &&
                (peopleData as any).length > 0 && (
                  <>
                    <h2 className="font-semibold text-xl leading-[25px]">
                      Results based on your search ({(peopleData as any).length}
                      )
                    </h2>
